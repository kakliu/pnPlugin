(function (factory) {
        ...
        fn.progressGet = function(li){
            ...
            var id = li.data("id");
            var cutPid = li.data("cutpid");

            proxy.progressNew(
                cutPid
                , function(data){
                if(typeof data.data === "number"){
                    li.find(".progress-bar").width(data.data * 100 + "%");
        ...
        fn.uploadFile = function(){
            ...
            $(".dropzone").dropzone({
                //指定上传图片的路径
                url: "http://image.panocooker.com/pano/upload",
                ...
                init: function () {
                    ...
                    submitButton.on("click", function (){
                        ...
                        if(pName == ""){
                            ...
                            if(options && options.data){
                                msg("请登陆后上传", function(){
                                    location.href = "http://login.panocooker.com?redirectURL=" + location.href;
                                });
                            } else {
                    ...
                    current.on("success", function (file, xhr) {
                        var scenePidObject = {};
                        scenePidObject.pid = xhr.data;
                        scenePidObject.sceneName = file.name;
                        scenePidArray.push(JSON.stringify(scenePidObject));
                        $(".addFileClas .cancel-file").hide();
                        $(".addFileClas .up-file").show();
                        $(".addFileClas .aginAddpic").hide();
                        if (current.getUploadingFiles().length === 0 && this.getQueuedFiles().length === 0) {
                            ...
                            proxy.panoUploadInEdit({
                                groupId: groupId,
                                panoId: panoId,
                                pids:'['+scenePidArray.toString()+']'
                            }, function(data){
